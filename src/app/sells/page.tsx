...
export default function SellsPage() {
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((product) => {
      ...
      return matchesSearch && matchesCondition && matchesPrice && matchesEra;
    });
      ...
      <section className="mx-auto max-w-7xl px-6 py-10 lg:px-8">
        <div className="mb-8">
          <h1 className="font-serif text-3xl font-bold">
            Results for <span className="text-primary">"{search || "All Finds"}"</span>
          </h1>
          ...
          <aside className="h-fit rounded-lg border border-border bg-surface p-5">
            ...
            <div className="mt-7">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {["All", "Excellent", "Very Good", "Good", "Mint"].map((item) => (
                  <button
                    key={item}
                    type="button"
            ...
            <div className="mt-7">
              ...
              <div className="mt-3 space-y-2">
                {["All", "1970s", "1980s", "1990s", "2000s"].map((item) => (
                  <label key={item} className="flex cursor-pointer items-center gap-2 font-serif text-xs text-muted">
                    <input
                      type="radio"
                      name="era"
                      value={item}
          ...
          <div>
            {filteredProducts.length === 0 ? (
              <div className="rounded-lg border border-border bg-surface p-16 text-center">
                ...
                <h2 className="mt-5 font-serif text-xl font-semibold">No finds matched your search</h2>
                <p className="mt-2 font-serif text-sm text-muted">Try another search or remove some filters.</p>
                <button
                  type="button"
                  onClick={() => {
                    setSearch("");
                  ...
                  {filteredProducts.map((product) => (
                    ...
                    >
                      ...
                      <div className="p-4">
                        ...
                        <h3 className="mt-1 truncate font-serif text-lg font-semibold">{product.name}</h3>
                        <div className="mt-3 flex items-center justify-between">
                          <p className="font-serif font-medium">${product.price.toLocaleString()}</p>
